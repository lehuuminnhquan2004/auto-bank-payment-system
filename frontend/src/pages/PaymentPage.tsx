...
export function PaymentPage() {
  ...
  useEffect(() => {
    ...
    const expiredAt = new Date(paymentExpiredAt).getTime();
    ...
    <main className="payment-page" data-status={payment.status}>
      ...
      {payment.status === 'PENDING' && (
        <>
          ...
          <img src={payment.qrUrl} alt="Mã QR chuyển khoản thanh toán" />

          <section className="transfer-details">
            <h2>Thông tin chuyển khoản</h2>

            <p>
              Ngân hàng: <strong>{payment.bank.bankId}</strong>
            </p>

            <p>
              Số tài khoản: <strong>{payment.bank.accountNo}</strong>
            </p>
            ...
            <p>
              Nội dung chuyển khoản: <strong>{payment.paymentCode}</strong>
            </p>
          </section>
          ...
          <section className="payment-progress" aria-live="polite">
            ...
            <p>Đang chờ xác nhận giao dịch ngân hàng...</p>
          </section>
        </>
      )}
      ...
      {payment.status === 'PAID' && (
        <section className="payment-result" role="status">
          ...
          <p>
            Mã thanh toán: <strong>{payment.paymentCode}</strong>
          </p>
      ...
      {payment.status === 'EXPIRED' && (
        <section className="payment-result" role="status">
          ...
          <p>
            Yêu cầu này đã hết hạn. Vui lòng tạo yêu cầu mới để tiếp tục thanh
            toán.
          </p>
      ...
      {payment.status === 'FAILED' && (
        <section className="payment-result" role="status">
          ...
          <p>
            Không thể hoàn tất thanh toán này. Vui lòng tạo yêu cầu mới để thử
            lại.
          </p>
